...
const GITHUB_SEARCH_QUERY_STRING =
    'path:/+filename:tooth.json+"format_version":+2+"tooth":+"version"+"info":+"name":+"description":+"author":+"tags":';
...
export class GitHubBot {
  ...
  async fetchAndSaveToothInfoJob_() {
    ...
    while (isLastPage === false) {
      let response = null;
      try {
        consola.debug(`[GitHub Bot] Octokit_.rest.search.code({q: '${
            GITHUB_SEARCH_QUERY_STRING}', per_page: 100, page: ${page}})`);
      ...
      for (const item of response.data.items) {
        ...
        subJobPromiseList.push(this.fetchAndSaveToothInfoSubJob_(owner, repo));
      }

      page++;
    }

    await Promise.all(subJobPromiseList);
  ...
  async fetchAndSaveToothInfoSubJob_(owner, repo) {
    ...
    await this.toothModel_.upsert({
      ...
      source: toothMetadata.getSource(),
...
async function fetchToothMetadata(owner, repo) {
  const toothJsonUrl =
      `https://cdn.jsdelivr.net/gh/${owner}/${repo}/tooth.json`;
