...
export class Version {
   ...
   */
  isStable() {
    return this.preReleaseName_ === undefined && this.major_ !== 0;
  }
...
/**
 * Gets the latest version from the given version list.
 * @param {Version[]} versionList The version list.
 * @returns {Version} The latest version.
 */
export function getLatestVersionFromVersionList(versionList) {
  if (versionList.length === 0) {
    throw new Error('version list is empty');
  }

  let /** @type {Version} */ greatestVersion = versionList[0];
  for (const version of versionList) {
    if (version.isGreaterThan(greatestVersion)) {
      greatestVersion = version;
    }
  }

  let /** @type {Version|undefined} */ greatestStableVersion;
  for (const version of versionList) {
    if (version.isStable() &&
        (greatestStableVersion === undefined ||
         version.isGreaterThan(greatestStableVersion))) {
      greatestStableVersion = version;
    }
  }

  if (greatestStableVersion !== undefined) {
    return greatestStableVersion;
  } else {
