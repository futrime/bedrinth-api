 ...
 * @throws {Error}
 */
function validateVersionParameters(
    major, minor, patch, preReleaseName, preReleaseNumber) {
  if (major < 0 || minor < 0 || patch < 0) {
    throw new Error('Numbers of a version must be non-negative.');
  }

  if (patch !== 0 &&
      (preReleaseName !== undefined || preReleaseNumber !== undefined)) {
    throw new Error('Patch versions cannot have pre-release identifiers.');
  }

  if (preReleaseName === undefined && preReleaseNumber !== undefined) {
    throw new Error('Pre-release number cannot be used without a name.');
  }
}
